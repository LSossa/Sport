...
router.get('/', (req, res) => {
  ...
  if (week) return res.json(db.prepare('SELECT * FROM vitamins WHERE date >= ? AND date < ? ORDER BY date, logged_at').all(week, addDays(week as string, 7)));
...
router.post('/', (req, res) => {
  ...
  const row = db.prepare('SELECT * FROM vitamins WHERE id = ?').get(result.lastInsertRowid);
  const { newBadges } = awardXp('vitamins', Number(result.lastInsertRowid), d.date);
