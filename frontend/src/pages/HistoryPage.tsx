...
export function HistoryPage() {
  ...
  const Row = ({ label, onDelete }: { label: string; onDelete: () => void }) => (
    <div className="flex items-center justify-between py-2 border-b border-slate-700 last:border-0">
      ...
      <button onClick={onDelete} className="p-3 text-red-400 hover:text-red-300"><Trash2 size={16} /></button>
    </div>
  );
  ...
  return (
    <Layout title={format(date, 'EEEE, MMM d')}>
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setDate(subDays(date, 1))} className="p-3 rounded-lg hover:bg-slate-700"><ChevronLeft size={20} className="text-slate-400" /></button>
        <span className="text-slate-300 font-medium">{format(date, 'MMMM d, yyyy')}</span>
        <button onClick={() => setDate(addDays(date, 1))} className="p-3 rounded-lg hover:bg-slate-700"><ChevronRight size={20} className="text-slate-400" /></button>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Section title="Shakes" icon="🥤" empty={data.shakes.length === 0}>
            {data.shakes.map(s => (
              <Row key={s.id} label={s.name} onDelete={() => setConfirm({ type: 'shake', id: s.id })} />
            ))}
          </Section>
